...
`order_notes` + `order-notes.functions.ts` + `/admin/sales/notes`, pridávajú sa dialógom
`components/admin/OrderNotesDialog.tsx` priamo pri objednávke v Predaji. Poznámka je **interná** —
...
`event_groups` + `events.group_id` + `event-groups.functions.ts` + `/admin/data/groups`. Série,
festivalové ročníky, predplatné cykly. Väzba je **1:N** — podujatie patrí najviac do jednej
skupiny, preto priradenie v dialógu najprv celú skupinu vyprázdni a potom nastaví vybrané
podujatia.
