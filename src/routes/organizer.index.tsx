...
export const Route = createFileRoute("/organizer/")({
  head: () => ({ meta: [{ title: "Organizer dashboard · eticketo.eu" }] }),
...
function OrganizerHome() {
  ...
  const totalCapacity = events.reduce(
    (s, e) => s + e.tickets.reduce((a, t) => a + t.quantity, 0),
    0,
  );
  const totalRevenue = events.reduce(
    (s, e) => s + e.tickets.reduce((a, t) => a + t.price * t.quantity, 0),
    0,
  );
    ...
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-4xl font-bold tracking-tight">
          {t("orgHome.welcome", { name: user?.full_name || t("orgHome.organizerFallback") })}
        </h1>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Button asChild variant="ghost" size="sm">
            <Link to="/organizer/events">{t("orgHome.allEvents")}</Link>
          </Button>
        </div>
        {events.length === 0 ? (
          <Card className="p-12 text-center bg-card/60 border-dashed border-border/50">
            ...
            <Button asChild className="mt-5 bg-gradient-flame text-primary-foreground shadow-glow">
              <Link to="/organizer/events/new">
                <Plus className="size-4 mr-2" /> {t("orgHome.addEventButton")}
              </Link>
            </Button>
          </Card>
        ) : (
              ...
              <Card key={e.id} className="overflow-hidden bg-card/60 border-border/50">
                ...
                <div className="p-5 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {e.status === "published"
                        ? t("orgHome.statusPublished")
                        : t("orgHome.statusDraft")}
                    </span>
                  ...
                  {e.status === "published" ? (
                    <Button asChild variant="ghost" size="sm" className="mt-2">
                      <Link to="/events/$id" params={{ id: e.id }}>
                        <Eye className="size-3.5 mr-1.5" /> {t("orgHome.view")}
                      </Link>
                    </Button>
                  ) : (
                    <div className="text-xs text-muted-foreground flex items-center gap-1.5 mt-2">
                      <FileText className="size-3.5" /> {t("orgHome.notPublic")}
                    </div>
                  )}
                </div>
              </Card>
...
function ActionCard({
  ...
  return (
    <Link to={to} className="block group">
      <Card
        className={`p-6 h-full transition border-border/50 ${
          primary
      ...
      >
        <div
          className={`size-12 rounded-xl grid place-items-center mb-4 ${primary ? "bg-background/15" : "bg-primary/15 text-primary"}`}
        >
          {icon}
        </div>
        <div className="font-display text-lg font-bold">{title}</div>
        <p
          className={`text-sm mt-1 ${primary ? "text-primary-foreground/80" : "text-muted-foreground"}`}
        >
          {desc}
        </p>
        <div
          className={`mt-4 inline-flex items-center gap-1 text-xs font-semibold ${primary ? "" : "text-primary"}`}
        >
          {t("orgHome.open")}{" "}
