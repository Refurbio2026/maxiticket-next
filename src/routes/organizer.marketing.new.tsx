...
const GOALS: { id: CampaignGoal; labelKey: string; descKey: string }[] = [
  ...
  { id: "remarketing", labelKey: "orgMktNew.goalRemarketingLabel", descKey: "orgMktNew.goalRemarketingDesc" },
  { id: "awareness", labelKey: "orgMktNew.goalAwarenessLabel", descKey: "orgMktNew.goalAwarenessDesc" },
];
...
function WizardPage() {
  ...
  const launch = () => {
    ...
    if (!platformConnected) {
      toast.error(t("orgMktNew.toastAccountNotConnected", { platform: platform === "google" ? "Google Ads" : "Meta Ads" }));
      return;
    }
    const c: Campaign = {
    ...
    return (
      <div className="p-6">
        <Card className="p-8 text-center space-y-4">
          ...
          <Button onClick={() => navigate({ to: "/organizer/events/new" })}>{t("orgMktNew.addEvent")}</Button>
        </Card>
      </div>
    );
  }
      ...
      <Card className="p-6">
        {step === 1 && (
          <div className="space-y-4">
            <h3 className="font-semibold flex items-center gap-2"><Calendar className="h-4 w-4 text-primary" /> {t("orgMktNew.step1Title")}</h3>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <PlatformBtn active={platform === "google"} onClick={() => setPlatform("google")} title="Google Ads" sub={google ? t("orgMktNew.connected") : t("orgMktNew.notConnected")} />
                <PlatformBtn active={platform === "meta"} onClick={() => setPlatform("meta")} title="Meta Ads" sub={meta ? t("orgMktNew.connected") : t("orgMktNew.notConnected")} />
              </div>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4">
            <h3 className="font-semibold flex items-center gap-2"><Target className="h-4 w-4 text-primary" /> {t("orgMktNew.step3Title")}</h3>
        ...
        {step === 4 && (
          <div className="space-y-4">
            <h3 className="font-semibold flex items-center gap-2"><Users className="h-4 w-4 text-primary" /> {t("orgMktNew.step4Title")}</h3>
        ...
        {step === 5 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold flex items-center gap-2"><Eye className="h-4 w-4 text-primary" /> {t("orgMktNew.step5Title")}</h3>
              ...
              <div className="rounded-xl border bg-card p-4">
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{t("orgMktNew.sponsored")}</div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("orgMktNew.headlines", { count: creative.headlines.length })}</Label>
                <Textarea rows={8} value={creative.headlines.join("\n")}
                  onChange={(e) => setCreative({ ...creative, headlines: e.target.value.split("\n") })} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">{t("orgMktNew.descriptions", { count: creative.descriptions.length })}</Label>
      ...
      {labels.map((l, i) => {
        ...
        return (
          <div key={l} className="flex flex-1 items-center gap-2">
            ...
            <span className={`text-xs ${active ? "font-medium" : "text-muted-foreground"}`}>{t(l)}</span>
