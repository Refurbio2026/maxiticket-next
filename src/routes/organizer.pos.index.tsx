...
function PosPage() {
  ...
  const applyPromo = () => {
    ...
    if (map[code]) { setDiscountPct(map[code]); toast.success(t("orgPos.promoApplied", { pct: map[code] })); }
    else { toast.error(t("orgPos.invalidPromo")); }
  };
    ...
    return (
      <div className="space-y-6">
        ...
        <Card className="p-8 bg-card/60 border-border/50 max-w-2xl">
          <div className="text-xs uppercase tracking-wider text-muted-foreground mb-3">{t("orgPos.selectEvent")}</div>
          <select
            value={eventId}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <Badge variant="outline" className="gap-1.5">
            ...
            {t("orgPos.terminalLabel")} {terminalStatus === "connected" ? t("orgPos.terminalConnectedShort") : terminalStatus === "busy" ? t("orgPos.terminalBusy") : t("orgPos.terminalDisconnected")}
          </Badge>
          {terminalStatus !== "connected" && (
            <Button size="sm" variant="outline" onClick={connect}>{t("orgPos.connectTerminal")}</Button>
          )}
          <Button asChild variant="outline" size="sm">
            <Link to="/organizer/pos/closing"><FileText className="size-4 mr-2" /> {t("orgPos.dailyClosing")}</Link>
          </Button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <Stat label={t("orgPos.statRevenue")} value={`€${(todayStats ? todayStats.cash_total + todayStats.card_total + todayStats.transfer_total : 0).toLocaleString("sk-SK")}`} icon={<TrendingUp className="size-4 text-primary" />} />
        <Stat label={t("orgPos.statCash")} value={`€${todayStats?.cash_total.toLocaleString("sk-SK") ?? 0}`} icon={<Banknote className="size-4 text-primary" />} />
        <Stat label={t("orgPos.statCard")} value={`€${todayStats?.card_total.toLocaleString("sk-SK") ?? 0}`} icon={<CreditCard className="size-4 text-primary" />} />
        <Stat label={t("orgPos.statTickets")} value={String(todayStats?.tickets_count ?? 0)} icon={<TicketIcon className="size-4 text-primary" />} />
        <Stat label={t("orgPos.statVoids")} value={`€${todayStats?.voided_total.toLocaleString("sk-SK") ?? 0}`} icon={<Ban className="size-4 text-destructive" />} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card className="p-5 bg-card/60 border-border/50">
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-2">{t("orgPos.selectEvent")}</div>
            {events.length === 0 ? (
              <p className="text-sm text-muted-foreground">{t("orgPos.noEvents")} <Link to="/organizer/events/new" className="text-primary underline">{t("orgPos.addEvent")}</Link></p>
            ) : (
          ...
          <Card className="p-5 bg-card/60 border-border/50">
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-3">{t("orgPos.ticketTypes")}</div>
            {!selectedEvent ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {selectedEvent.tickets.map((tk) => (
                  ...
                  >
                    ...
                    <div className="text-[11px] text-muted-foreground mt-1">{t("orgPos.capacity", { count: tk.quantity })}</div>
                    <div className="mt-3 text-xs text-primary opacity-0 group-hover:opacity-100 transition">{t("orgPos.addToCart")}</div>
                  </button>
                ))}
              </div>
            )}
        ...
        <Card className="p-5 bg-card/60 border-border/50 flex flex-col h-fit lg:sticky lg:top-24">
          <div className="text-xs uppercase tracking-wider text-muted-foreground mb-3">{t("orgPos.cart")}</div>
          <div className="space-y-2 max-h-[300px] overflow-y-auto">
            {cart.length === 0 ? (
              <p className="text-sm text-muted-foreground py-8 text-center">{t("orgPos.cartEmpty")}</p>
            ) : cart.map((i) => (
          ...
          <div className="space-y-2">
            <div className="flex gap-2">
              <Input placeholder={t("orgPos.promoPlaceholder")} value={promo} onChange={(e) => setPromo(e.target.value)} className="h-9" />
              <Button variant="outline" size="sm" onClick={applyPromo}>{t("orgPos.apply")}</Button>
            </div>
            <div className="flex justify-between text-sm"><span className="text-muted-foreground">{t("orgPos.subtotal")}</span><span>€{subtotal.toFixed(2)}</span></div>
            {discount > 0 && (
              <div className="flex justify-between text-sm text-primary"><span>{t("orgPos.discount", { pct: discountPct })}</span><span>−€{discount.toFixed(2)}</span></div>
            )}
            <div className="flex justify-between font-display text-2xl font-bold pt-1">
              <span>{t("orgPos.total")}</span><span>€{total.toFixed(2)}</span>
            </div>
          </div>

          <Separator className="my-4" />
          <div className="grid grid-cols-2 gap-2">
            <PayBtn icon={<Banknote className="size-5" />} label={t("orgPos.payCash")} onClick={() => checkout("cash")} disabled={processing || cart.length === 0} />
            <PayBtn icon={<CreditCard className="size-5" />} label={t("orgPos.payCard")} onClick={() => checkout("card")} disabled={processing || cart.length === 0} primary />
            <PayBtn icon={<Building2 className="size-5" />} label={t("orgPos.payTransfer")} onClick={() => checkout("transfer")} disabled={processing || cart.length === 0} />
            <PayBtn icon={<Gift className="size-5" />} label={t("orgPos.payGuestlist")} onClick={() => checkout("free")} disabled={processing || cart.length === 0} />
          </div>
        </Card>
      </div>
      ...
      <Card className="p-5 bg-card/60 border-border/50">
        <div className="flex items-center justify-between mb-3">
          <div className="text-xs uppercase tracking-wider text-muted-foreground">{t("orgPos.recentSales")}</div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-xs text-muted-foreground border-b border-border/50">
                <tr><th className="text-left py-2">{t("orgPos.thReceipt")}</th><th className="text-left">{t("orgPos.thEvent")}</th><th className="text-left">{t("orgPos.thPayment")}</th><th className="text-right">{t("orgPos.thAmount")}</th><th className="text-left">{t("orgPos.thStatus")}</th><th className="text-left">{t("orgPos.thCashier")}</th><th></th></tr>
              </thead>
              <tbody>
                {recent.map((s) => (
      ...
      <Dialog open={!!lastSale} onOpenChange={(o) => !o && setLastSale(null)}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("orgPos.ticketsDialogTitle", { number: lastSale?.receipt_number })}</DialogTitle>
          </DialogHeader>
          {lastSale && (() => {
            ...
            return (
              <div className="space-y-4 text-sm">
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  ...
                  <span>{t("orgPos.totalLabel")} <strong className="text-foreground">€{lastSale.total.toFixed(2)}</strong></span>
                </div>
                  ...
                  {saleTickets.map((tk, idx) => (
                    <div key={tk.id} className="rounded-xl border border-border/50 bg-background p-3 flex gap-3 items-center">
                      <div className="aspect-square w-20 bg-white rounded-md p-1 flex items-center justify-center shrink-0">
                        <img alt={t("orgPos.qrAlt")} className="w-full h-full" src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${encodeURIComponent(tk.code)}`} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{t("orgPos.ticketNumber", { num: idx + 1 })}</div>
                        ...
                        <Badge variant="outline" className="mt-1 text-[10px] uppercase">{tk.status}</Badge>
                      </div>
                    </div>
                  ))}
                </div>
                <DialogFooter className="gap-2 sm:gap-2 flex-wrap">
                  ...
                  >
                    <Printer className="size-4 mr-1.5" /> {t("orgPos.printAllTickets", { count: saleTickets.length })}
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => toast.success(t("orgPos.ticketsEmailed"))}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              {t("orgPos.orpStatusLabel")} <span className={connected ? "text-foreground font-medium" : ""}>{connected ? t("orgPos.orpConnected") : status === "error" ? t("orgPos.orpError") : t("orgPos.orpDisconnected")}</span>
              {last && <> · {t("orgPos.orpLastReceipt")} <span className="font-mono">{last.receipt_number}</span></>}
            </div>
          </div>
        </div>
        <Button asChild variant="outline" size="sm">
