...
export const Route = createFileRoute("/admin/sales/sales")({
  head: () => ({ meta: [{ title: "Predaj · eticketo.eu Admin" }] }),
  component: Page,
});

const fmtEur = (n: number, c = "EUR") =>
  `${c === "EUR" ? "€ " : ""}${n.toLocaleString("sk-SK", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
                  ...
                  return (
                    <tr key={r.id} className="border-b border-border/20 hover:bg-muted/30">
                      ...
                      <td className="p-3 text-right font-semibold">
                        {fmtEur(r.total_amount, r.currency)}
                      </td>
                      <td className="p-3 text-center">{r.qty}</td>
                      <td className="p-3">
                        <Badge
                          className={`${st?.cls || "bg-muted text-muted-foreground"} border-0`}
                        >
                          {st?.label || r.status}
                        </Badge>
                      </td>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="notify" className="cursor-pointer">
                Odoslať potvrdenie zákazníkovi emailom
              </Label>
            </div>
          </div>
