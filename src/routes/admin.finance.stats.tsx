...
export const Route = createFileRoute("/admin/finance/stats")({
  head: () => ({ meta: [{ title: "Finančné štatistiky · eticketo.eu Admin" }] }),
  component: Page,
});
...
function Page() {
  ...
  const kpi = [
    {
      label: "Zaplatené (suma)",
      value: d ? fmtEur(d.total_paid_amount) : "—",
      sub: d ? `${fmtNum(d.total_paid_orders)} objednávok` : "",
      icon: Wallet,
      tone: "primary",
    },
    {
      label: "Čaká na platbu",
      value: d ? fmtEur(d.total_pending_amount) : "—",
      sub: d ? `${fmtNum(d.total_pending_orders)} objednávok` : "",
      icon: Clock,
      tone: "accent",
    },
    {
      label: "Refundované",
      value: d ? fmtEur(d.total_refunded_amount) : "—",
      sub: d ? `${fmtNum(d.total_refunded_orders)} objednávok` : "",
      icon: RotateCcw,
      tone: "destructive",
    },
    {
      label: "Vystavené faktúry",
      value: d ? fmtNum(d.invoices_issued) : "—",
      sub: d && d.invoices_failed ? `${d.invoices_failed} zlyhalo` : "OK",
      icon: FileCheck2,
      tone: "success",
    },
  ];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Agregované finančné ukazovatele zo systému.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {kpi.map((s) => (
          <Card key={s.label} className="relative overflow-hidden border-border/50 bg-card">
            <div
              className={`absolute inset-0 bg-gradient-to-br ${toneClass[s.tone].split(" ").slice(0, 2).join(" ")} opacity-40`}
            />
            <CardContent className="relative p-5">
              <div className="flex items-start justify-between">
                <div
                  className={`flex h-9 w-9 items-center justify-center rounded-lg bg-background/60 ${toneClass[s.tone].split(" ").slice(2).join(" ")}`}
                >
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      formatter={(v: number, k: string) => [
                        k === "revenue" ? fmtEur(v) : fmtNum(v),
                        k === "revenue" ? "Tržby" : "Objednávky",
                      ]}
                    />
