...
function ArtistsPage() {
  ...
  const list = useMemo(
    () =>
      ARTISTS.filter(
        (a) => (cat === "all" || a.cat === cat) && a.name.toLowerCase().includes(q.toLowerCase()),
      ),
    [q, cat],
  );
  return (
      ...
      <div className="space-y-6">
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {list.map((a) => (
            <Card
              key={a.id}
              className="group overflow-hidden bg-card/60 border-border/60 hover:border-primary/50 transition-all hover:-translate-y-0.5"
            >
