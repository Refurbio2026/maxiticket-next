...
export const Route = createFileRoute("/admin/pos/terminals")({
  ...
  component: () => {
    ...
    useEffect(() => { setList(getDevices()); }, [tick]);
    useEffect(() => {
    ...
    return (
      <div className="space-y-6">
        ...
        <p className="text-sm text-muted-foreground">Mock prehľad. Reálne napojenie cez WebUSB / bridge daemon poskytovateľa.</p>
        <Card className="p-5 bg-card/60 border-border/50">
          {list.length === 0 ? <p className="text-sm text-muted-foreground">Žiadne zariadenia.</p> : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
              {list.map((d) => (
                <Card key={d.id} className="p-4 bg-background border-border/50">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="font-display font-semibold flex items-center gap-2"><Usb className="size-4" /> {d.name}</div>
                      <div className="text-xs text-muted-foreground">{d.location || "—"}</div>
                      <div className="text-[10px] font-mono text-muted-foreground mt-1">org: {d.organizer_id}</div>
                    </div>
