...
function Page() {
  ...
  // Kategórie sú zatiaľ v localStorage, podujatia už v databáze — počty
  // v kategóriách preto skladáme z oboch zdrojov.
  const { data: events = [] } = useEvents({ scope: "all" });

  const load = () => setCats(getCategories());
  ...
  useEffect(() => {
    ...
    for (const c of cats) map[c.id] = events.filter((e) => e.category === c.name).length;
    setCounts(map);
  }, [cats, events]);
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const slug = (form.slug.trim() || name.toLowerCase().replace(/\s+/g, "-")).replace(
      /[^a-z0-9-]/g,
      "",
    );
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Hlavné kategórie (Koncert, Šport, …) a podujatia v nich.
          </p>
        </div>
