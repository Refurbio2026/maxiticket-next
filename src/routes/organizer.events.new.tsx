...
function NewEventPage() {
    ...
    return (
      <Card className="p-12 text-center bg-card/60 border-dashed border-border/50 max-w-2xl">
        <h2 className="font-display text-2xl font-semibold mb-2">
          {t("orgEventsNew.noPermissionTitle")}
        </h2>
        <p className="text-muted-foreground">{t("orgEventsNew.noPermissionText")}</p>
      </Card>
    );
  }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      // `organizer_id` neposielame — server ho odvodí z prihláseného používateľa,
      // aby si nikto nemohol založiť podujatie pod cudzím organizátorom.
  ...
  return (
    <form onSubmit={submit} className="space-y-6 max-w-4xl">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-display text-lg font-semibold">
              {t("orgEventsNew.ticketTypesTitle")}
            </h2>
      ...
      <div className="flex justify-end gap-3">
        <Button type="button" variant="outline" onClick={() => navigate({ to: "/organizer" })}>
          {t("orgEventsNew.cancel")}
        </Button>
        <Button
          type="submit"
          disabled={busy}
    ...
    <div className="grid sm:grid-cols-[1fr_120px_120px_auto] gap-2 items-end">
      <div className="space-y-1">
        ...
        <Input
          value={ticket.name}
          onChange={(e) =>
            setTickets((arr) => arr.map((x, j) => (j === i ? { ...x, name: e.target.value } : x)))
          }
        />
      </div>
      <div className="space-y-1">
        ...
        <Input
          ...
          value={ticket.price}
          onChange={(e) =>
            setTickets((arr) => arr.map((x, j) => (j === i ? { ...x, price: e.target.value } : x)))
          }
        />
      </div>
      <div className="space-y-1">
        ...
        <Input
          ...
          onChange={(e) =>
            setTickets((arr) =>
              arr.map((x, j) => (j === i ? { ...x, quantity: e.target.value } : x)),
            )
          }
        />
      </div>
